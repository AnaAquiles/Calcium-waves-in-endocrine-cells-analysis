...
def ContourTimeSeries(Stack, ROI_dict, NoFrames, alto, ancho, diametro):
    ...
    for j in range(NoContornos):            
        centro = centros[j]
        SerieTiempo = ContourTimeSerie(Stack, centro, NoFrames, alto, ancho,\
                                       diametro)
...
#Genera una serie de tiempo dado un stack de datos, un punto central, 
#un diámetro, etc
def ContourTimeSerie(Stack, centro, NoFrames, alto, ancho, diametro):
    ...
    cv2.circle(binaria,(int(centro[0]),int(centro[1])), diametro, \
               (255,255,255), -1)
