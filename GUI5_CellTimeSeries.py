...
def ContourTimeSeries(Stack, ROI_dict, NoFrames, alto, ancho, diametro):
    
    centros= list(ROI_dict.values())                                         #Los contornos se obtienen a partir del diccionario, pero deben pasarse a una lista, si no la variable es tipo "dict_values"
    NoContornos = len(centros)                     

    TimeSer_dict = {}                                                          #Diccionario que contendrá las series de tiempo
    for j in range(NoContornos):            
        centro = centros[j]
        SerieTiempo = ContourTimeSerie(Stack, centro, NoFrames, alto, ancho, diametro)
...
#Genera una serie de tiempo dado un stack de datos, un punto central, un diámetro, etc
def ContourTimeSerie(Stack, centro, NoFrames, alto, ancho, diametro):

    binaria = np.zeros((alto, ancho));                                     #Máscara binaria para obtener la serie de tiempo
    cv2.circle(binaria,(int(centro[0]),int(centro[1])), diametro, (255,255,255), -1)
